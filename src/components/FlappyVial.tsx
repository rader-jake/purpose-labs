...
export function FlappyVial() {
  ...
  const flap = useCallback(() => {
    ...
    if (p === "dying") return;
    if (p === "dead") { phaseRef.current = "ready"; return; }
    ...
    function hitTest(): boolean {
      ...
      for (const p of pipes.current) {
        if (bx + HIT_W / 2 > p.x + 6 && bx - HIT_W / 2 < p.x + PIPE_W - 6) {
          if (by - HIT_H / 2 < p.gapY - PIPE_GAP / 2 || by + HIT_H / 2 > p.gapY + PIPE_GAP / 2) return true;
        }
      }
      return false;
    }
    ...
    function loop(ts: number) {
      ...
      if (phase === "playing") {
        ...
        if (ts - lastPipe.current > PIPE_INTERVAL) {
          pipes.current.push({ x: W, gapY: (H - GROUND_H) * 0.22 + Math.random() * (H - GROUND_H) * 0.56, scored: false });
          lastPipe.current = ts;
        }
        const bx = W * 0.28;
        ...
        for (const p of pipes.current) {
          p.x -= PIPE_SPEED;
          if (!p.scored && p.x + PIPE_W < bx) { p.scored = true; scoreRef.current++; }
        }
        if (hitTest()) {
      ...
      if (phase === "playing" || phase === "dying") drawScore(scoreRef.current, W / 2, 18);
